<?php
// Auto-load lang-switcher si pas encore chargé
if (!function_exists('t')) {
    require_once __DIR__ . '/../lang/lang-switcher.php';
}
...
.lang-option .opt-flag { font-size: 18px; }
.lang-option .opt-label { flex: 1; }
.lang-option .opt-code { font-size: 10px; color: #6b7280; letter-spacing: 0.06em; }
...
@media (max-width: 768px) {
    ...
    .navbar-pill-wrap { position: static; transform: none; }
...
<?php
// Flags + labels pour le switcher
$langs_info = [
    'fr' => ['flag' => '🇫🇷', 'label' => 'Français'],
    'en' => ['flag' => '🇬🇧', 'label' => 'English'],
    'ar' => ['flag' => '🇲🇦', 'label' => 'العربية'],
];
$current_lang_info = $langs_info[$lang_code];
?>
    ...
    <div class="navbar-inner">
        <div class="navbar-content">
            ...
            <?php if (isset($_SESSION['user_id'])): ?>
                ...
                <a href="../notifications.php" class="nav-link nav-icon-only" title="<?= t('nav_notifications') ?>">
                ...
                <a href="../favorites.php" class="nav-link nav-icon-only" title="<?= t('nav_favorites') ?>">
                ...
                <div class="nav-dropdown">
                    <a href="../profile/profile.php" class="nav-link">
                        ...
                        <span><?= htmlspecialchars($_SESSION['username'] ?? t('nav_profile')) ?></span>
                    </a>
                    <div class="dropdown-menu">
                        <a href="../profile/profile.php">👤 <?= t('nav_profile') ?></a>
                        <a href="../booking/my-bookings.php">📋 <?= t('nav_bookings') ?></a>
                        <a href="../profile/loyalty-points.php">⭐ <?= t('nav_loyalty') ?></a>
                        <?php if (isset($_SESSION['role']) && $_SESSION['role'] === 'admin'): ?>
                            <a href="../admin/dashboard.php">⚙️ <?= t('nav_admin') ?></a>
                        <?php endif; ?>
                        <a href="../auth/logout.php" class="logout">🚪 <?= t('nav_logout') ?></a>
                    </div>
                </div>
            ...
            <!-- Lang switcher intégré dans la navbar -->
            <div class="lang-switcher">
                <button class="lang-trigger">
                    <span class="lang-flag"><?= $current_lang_info['flag'] ?></span>
                    <span class="lang-code"><?= strtoupper($lang_code) ?></span>
                </button>
                <div class="lang-dropdown">
                    <?php foreach ($langs_info as $code => $info): ?>
                        <a href="?lang=<?= $code ?>"
                           class="lang-option <?= $lang_code === $code ? 'active-lang' : '' ?>">
                            <span class="opt-flag"><?= $info['flag'] ?></span>
                            <span class="opt-label"><?= $info['label'] ?></span>
                            <span class="opt-code"><?= strtoupper($code) ?></span>
                        </a>
                    <?php endforeach; ?>
                </div>
            </div>
