...
if (!$activity) {
    header("Location: activities.php?error=not_found");
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['confirm_delete'])) {
    ...
    try {
        ...
        $pdo->prepare("DELETE FROM favorites WHERE type = 'activity' AND reference_id = :id")
            ->execute([':id' => $id]);

        // 2. Delete related reviews
        $pdo->prepare("DELETE FROM reviews WHERE activity_id = :id")
            ->execute([':id' => $id]);

        // 3. Delete related bookings
        $pdo->prepare("DELETE FROM bookings WHERE type = 'activity' AND reference_id = :id")
            ->execute([':id' => $id]);

        // 4. Delete activity record
        $pdo->prepare("DELETE FROM activities WHERE id = :id")
            ->execute([':id' => $id]);
        ...
        if (!empty($activity['image'])) {
            $img_path = '../../uploads/activities/' . $activity['image'];
...
<head>
    ...
    <title>Supprimer l'activité — Admin</title>
...
<div class="admin-topbar">
    <div class="topbar-left">
        <div>
            <div class="topbar-title">Supprimer l'activité</div>
            <div class="topbar-breadcrumb">
                Home <span>/</span>
                <a href="activities.php" style="color:inherit;text-decoration:none;">Activités</a>
                <span>/</span> Supprimer
            </div>
        </div>
    </div>
...
<div class="admin-body">
    ...
    <div class="page-header">
        <div>
            <h1 style="color:var(--color-danger);">🗑️ Supprimer l'activité</h1>
            <p>Cette action est irréversible</p>
        </div>
        <a href="activities.php" class="btn btn-secondary">← Retour à la liste</a>
    </div>
                    ...
                    <div>
                        ...
                        <div style="display:flex;gap:.75rem;align-items:center;flex-wrap:wrap;">
                            <span style="font-weight:700;color:var(--ocean-teal);font-size:.9rem;">
                                <?= number_format($activity['price'], 0, ',', ' ') ?> MAD / nuit
                            </span>
                            <span style="color:#f59e0b;font-size:.85rem;">
                                ⭐ <?= number_format($activity['rating'], 1) ?>
                            </span>
                            <?php if (!empty($activity['type'])): ?>
                            <span class="badge badge-info"><?= htmlspecialchars(ucfirst($activity['type'])) ?></span>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
        ...
        <div class="card" style="border-color:rgba(214,69,69,0.2);">
            ...
            <div class="card-body">
                <p style="font-size:.875rem;color:var(--text-muted);margin-bottom:1.25rem;">
                    Pour confirmer, tapez le nom de l'activité ci-dessous :
                    <strong style="color:var(--text-primary);"><?= htmlspecialchars($activity['name']) ?></strong>
                </p>
                ...
                <form method="POST">
                    <div class="form-group" style="margin-bottom:1.25rem;">
                        <input
                            ...
                            type="text"
                            id="confirm-name"
                            placeholder="Tapez le nom de l'activité..."
                            autocomplete="off"
                    ...
                    <div style="display:flex;gap:.75rem;justify-content:flex-end;">
                        <a href="edit-activity.php?id=<?= $id ?>" class="btn btn-secondary">
                            ← Annuler
                        </a>
                        <button
                            type="submit"
...
const expected = <?= json_encode($activity['name']) ?>;
